...
int32_t width=1024;
int32_t height=1024;
int32_t maxiter=1048576L;
char logprefix[128];

static uint32_t *dumpbuffer=NULL;
static int32_t totpix_done=0;
static int32_t lasttotpix_done=0;
static double lastTotpixPercent=0;
...
int32_t mandel(FLOAT cx, FLOAT cy, int32_t maxiter) {
  int32_t i=maxiter;
...
#ifdef DEBUG
  fprintf(logfile,"mandel=%u, maxiter=%Lu\n",i,maxiter);
#endif
  return maxiter-i;
}
 
int32_t mandelspeed(FLOAT cx, FLOAT cy, int32_t maxiter) {
  int32_t i=maxiter;
  FLOAT zx=cx,zy=cy,zx2=cx*cx,zy2=cy*cy;
  ...
  fprintf(logfile,"mandel=%u, maxiter=%Lu\n",i,maxiter);
#endif
  return (int32_t)((long double)maxiter * sqrt((zx-ox)*(zx-ox)+(zy-oy)*(zy-oy)) / SPEED_MAX);
}
 
int32_t mandelangle(FLOAT cx, FLOAT cy, int32_t maxiter) {
  int32_t i=maxiter;
  FLOAT zx=cx,zy=cy,zx2=cx*cx,zy2=cy*cy;
  long double ox=cx,oy=cy;
  while((i>0)&&(zx2+zy2 < 4.0)) {
...
#endif
  return (int32_t)(maxiter*(3.1415926535+atan2((zx-ox),(zy-oy)))/6.28320);
}
...
*/ 
int32_t mandelde(FLOAT cx, FLOAT cy, int32_t maxiter, FLOAT k, int logr) {
  int32_t i=maxiter;
  FLOAT zx=cx,zy=cy,zx2=cx*cx,zy2=cy*cy;
  FLOAT dzx = 0, dzy = 0;
...
void *run_checkfilled(void *cfgi) {
  ...
  debug("Entering run_checkfilled");

  if (cfg->mode != MODE_NORMAL && cfg->mode != MODE_DE) {
    cfg->terminated=1;
    return NULL;
  }
  ...
  debug("Finished loop, filled=%d", filled);
...
int main(int argc,char *argv[]) {
  ...
  int32_t totpix=(int32_t)width*height;
  ...
  while ((o=getopt(argc, argv, "d:o:p:w:h:m:O:M:K:L")) != EOF) {
    ...
    case 'O': oldmaxiter=atol(optarg); break;
  ...
  */
  ...
  if (stat(dumpfilename, &fstatbuf) == 0) {
    ...
    if (wtmp != width || htmp != height) {
      fprintf(stderr, "Existing file has different width/height (%d,%d). Aborting.\n", wtmp, htmp);
      return -3;
    }
  ...
  if ((dumpbuffer=(uint32_t *)mmap(NULL,
			      sizeof(header) + 
  ...
  case THMAND_MODE_INITIALIZE:
    ...
    // This is why all address uses an offset of 4 ints (2 int32_ts) instead of 2.
    ...
    for (y=0; y<height; y++) {
      ...
      for (x=0; x<width; x++) {
	dumpbuffer[4 + x + y * width] = htonl((int32_t)-1);
      }
      msync(&(dumpbuffer[4 + y*width]), width * sizeof(uint32_t), MS_ASYNC);
    }
    fprintf(stderr, "Done.    \n");
    break;
  ...
  while (nthreads > 0 || workstack) {
    ...
    for (tempindex = 0; tempindex < ncores; tempindex++) {
      if (cfg[tempindex].taskid >=0) {
	//debug("Checking thread %d", tempindex);
      }
      if (cfg[tempindex].taskid >= 0 && cfg[tempindex].terminated) {
    ...
    if (totpix_done > lasttotpix_done) {
      double totpixPercent =  100 * (double)totpix_done / (double)totpix;
      snprintf(logprefix, sizeof(logprefix), "thmand: %g%% done (%d running threads): ", totpixPercent, nthreads);
#ifndef LOGGING
      if (totpixPercent - lastTotpixPercent >= 1) {
	fprintf(stderr, "%d%% done      \r", (int)totpixPercent);
	lastTotpixPercent = totpixPercent;
#endif
      }
  ...
  if (munmap(dumpbuffer, sizeof(header) + sizeof(uint32_t) * (width * height)) == -1) {
    perror("munmap()");
  }
